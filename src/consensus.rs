...
mod tests {
    ...
    async fn test_consensus_engine_creation() {
        ...
        let consensus = ConsensusEngine::new(blockchain, config, false)
            .await
            .unwrap();
        let metrics = consensus.get_metrics().await;

        assert_eq!(metrics.current_validators, 0);
        assert_eq!(metrics.total_stake, 0);
    }
    ...
    #[test]
    fn test_pow_hash_validation() {
        let storage = Arc::new(
            tokio_test::task::spawn(async { StorageManager::new_memory().await.unwrap() }).await,
        );
        let blockchain = Arc::new(RwLock::new(
        ...
        let config = ConsensusConfig::default();
        let consensus = tokio_test::task::spawn(async {
            ConsensusEngine::new(blockchain, config, false)
                .await
                .unwrap()
        })
        .await;
        ...
        assert!(consensus.validate_pow_hash("0000abcd", 4000));
        assert!(!consensus.validate_pow_hash("1234abcd", 4000));
    }
}
