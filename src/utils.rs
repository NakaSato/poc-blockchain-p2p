...
mod tests {
    ...
    fn test_energy_conversion() {
        assert_eq!(
            EnergyConversion::convert(1.0, EnergyUnit::KWh, EnergyUnit::Wh),
            1000.0
        );
        assert_eq!(
            EnergyConversion::convert(1000.0, EnergyUnit::Wh, EnergyUnit::KWh),
            1.0
        );
        assert_eq!(
            EnergyConversion::convert(1.0, EnergyUnit::MWh, EnergyUnit::KWh),
            1000.0
        );
    }
    ...
    fn test_token_conversion() {
        assert_eq!(EnergyConversion::energy_to_tokens(1.0), 1_000_000);
        assert_eq!(EnergyConversion::tokens_to_energy(1_000_000), 1.0);
    }

    #[test]
    fn test_thai_peak_hours() {
        // This test would need to mock the current time
        // Just test that the function doesn't panic
        let _is_peak = ThaiEnergyMarket::is_peak_hours();
    }

    #[test]
    fn test_grid_location_validation() {
        assert!(ThaiEnergyMarket::validate_grid_location("BKK-01-SUB001"));
        assert!(!ThaiEnergyMarket::validate_grid_location("INVALID"));
        assert!(!ThaiEnergyMarket::validate_grid_location("BKK-1-SUB001"));
    }

    #[test]
    fn test_province_codes() {
        assert_eq!(ThaiEnergyMarket::get_province_code("bangkok"), Some("BKK"));
        assert_eq!(
            ThaiEnergyMarket::get_province_code("chiang mai"),
            Some("CNX")
        );
        assert_eq!(ThaiEnergyMarket::get_province_code("unknown"), None);
    }

    #[test]
    fn test_carbon_footprint() {
        let coal_footprint = EnergyConversion::carbon_footprint(100.0, "coal");
        let solar_footprint = EnergyConversion::carbon_footprint(100.0, "solar");
        assert!(coal_footprint > solar_footprint);
    }

    #[test]
    fn test_data_validation() {
        assert!(DataValidation::validate_energy_amount(10.0).is_ok());
        assert!(DataValidation::validate_energy_amount(-1.0).is_err());
        assert!(DataValidation::validate_energy_amount(200_000.0).is_err());

        assert!(DataValidation::validate_energy_price(5000).is_ok());
        assert!(DataValidation::validate_energy_price(100).is_err());
        assert!(DataValidation::validate_energy_price(50_000).is_err());
    }
    ...
    fn test_crypto_keypair_generation() {
        let wallet = crypto::generate_keypair().unwrap();
        assert_eq!(wallet.public_key.len(), 32);
        assert_eq!(wallet.private_key.len(), 32);
        assert_eq!(wallet.address.len(), 40); // 20 bytes as hex
    }
    ...
    fn test_crypto_sign_verify() {
        ...
        let signature = crypto::sign_data(&wallet.private_key, data).unwrap();

        // Note: This simplified implementation won't properly verify
        // In a real implementation, you'd use proper cryptographic functions
