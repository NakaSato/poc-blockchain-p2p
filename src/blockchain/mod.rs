...
mod tests {
    ...
    fn test_blockchain_config_default() {
        ...
        assert_eq!(config.energy_token_ratio, 1.0);
        assert_eq!(config.target_block_time, 10);
        assert_eq!(config.max_transactions_per_block, 1000);
    }

    #[test]
    fn test_energy_token_conversion() {
        assert_eq!(utils::energy_to_tokens(1.0), 1_000_000);
        assert_eq!(utils::tokens_to_energy(1_000_000), 1.0);
        assert_eq!(utils::energy_to_tokens(0.5), 500_000);
    }

    #[test]
    fn test_carbon_credits_calculation() {
        assert_eq!(utils::calculate_carbon_credits(10.0, "solar"), 5.0);
        assert_eq!(utils::calculate_carbon_credits(10.0, "wind"), 6.0);
        assert_eq!(utils::calculate_carbon_credits(10.0, "coal"), 0.0);
    }
    ...
    fn test_address_validation() {
        ...
        assert!(!utils::validate_address("invalid_address"));
        assert!(!utils::validate_address("too_short"));
    }

    #[test]
    fn test_merkle_root_calculation() {
        let transactions = vec![];
        assert_eq!(utils::calculate_merkle_root(&transactions).unwrap(), "");

        // Test with actual transactions would require Transaction implementation
    }

    #[test]
    fn test_compliance_validation() {
        // This would require Transaction and energy transaction implementations
        // Will be tested when those modules are complete
    }
}
